...
#parser.add_argument("--buildlocation", help="build folder location (can be relative)",default="../build/afr-example-ota/CY8CPROTO-062-4343W/Debug", required=False)
parser.add_argument("--buildlocation", help="build folder location (can be relative)",default="../../../../../../build/cy/blinky_cm4/CY8CPROTO-062-4343W/Debug", required=False)
...
class AWS_IoT_OTA:
    ...
    def BuildFirmwareFileNames(self):
        ...
        self.BUILD_PATH=Path(args.buildlocation)
        # We Should have the versions stored at this point. Build the App name
        self.APP_NAME="blinky_cm4_v" + args.appversion + ".bin"
        self.APP_FULL_NAME=self.BUILD_PATH / Path(self.APP_NAME)
        self.BUILD_FILE_FULL_NAME=self.BUILD_PATH/Path("blinky_cm4.bin")
        print ("Name of binary for uploading to AWS: " + str(self.APP_FULL_NAME))
